 ...
 */
constexpr u32 M68K_INT_ACK_AUTOVECTOR    = 0xffffffff;

/* Causes the spurious interrupt vector (0x18) to be taken
 * This happens in a real 68K if BERR is asserted during the interrupt
 * acknowledge cycle (i.e. no devices responded to the acknowledge).
 */
constexpr u32 M68K_INT_ACK_SPURIOUS      = 0xfffffffe;
...
protected:
	...
	virtual u32 execute_input_lines() const override { return 8; }; // number of input lines
	virtual u32 execute_default_irq_vector(int inputnum) const override { return M68K_INT_ACK_AUTOVECTOR; }
...
public:
	...
	void set_buserror_details(u32 fault_addr, uint8_t rw, uint8_t fc);
...
//
	...
	int m_hmmu_enabled;  /* Indicates if the HMMU is enabled and the type of translation */

	u32 m_exception_pc;
	u16 m_exception_sr;
	u16 m_exception_vector;
...
public:
	// construction/destruction
	m68301_device(const machine_config &mconfig, const char *tag, device_t *owner, u32 clock);
...
public:
	// construction/destruction
	m68008plcc_device(const machine_config &mconfig, const char *tag, device_t *owner, u32 clock);
...
public:
	// construction/destruction
	scc68070_device(const machine_config &mconfig, const char *tag, device_t *owner, u32 clock);
